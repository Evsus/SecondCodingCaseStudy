...
class Lexer {
    ...
    /**
     * Provides sequential access to the sequence of lexemes. Repeatedly calling this method without passing false will return each lexeme in sequence, return null after the last lexeme, then start the sequence over again. Passing false allows the caller to "preview" the next lexeme without advancing past it.
     * @argument {boolean} [consume=true] - Indicates whether or not to move the current position in the sequence forward.
     * @modifies This lexer's current position in the sequence of lexemes, if <code>consume</code> is true.
    ...
    getLexeme(consume) {
        if (this._lexemeIndex >= this._lexemes.length) {
            if (consume !== false) this._lexemeIndex = 0;
            return null;
        }

        let result = this._lexemes[this._lexemeIndex];
        if (consume !== false) this._lexemeIndex++;
        return result;
    }
}
